...
class AnalysisCard(BaseCard):
    ...
    def _is_detail_line(self, line: str) -> bool:
        ...
        return (
            any(line.lower().startswith(starter) for starter in detail_starters)
            or ":" in line
    ...
    def render_sections_overview(self):
        ...
        if self.sections:
            ...
            st.success(
                f"**Successfully detected {len(self.sections)} sections from your resume!**"
            )
    ...
    def _format_content_simple(self, content: str) -> str:
        ...
        return (
            ...
            if formatted_lines
            else "*Content parsing in progress...*"
        )
    ...
    def _is_list_item(self, line: str) -> bool:
        ...
        return (
            any(line.lower().startswith(word) for word in action_words)
            or ":" in line
            or line.startswith(("•", "-", "*"))
        )
        ...
        try:
            ...
            for section_name, content in sections.items():
                ...
                if content_str and content_str.strip():
                    emoji = self._get_section_emoji(section_name)
                    st.markdown(
                        f"#### {emoji} {section_name.replace('_', ' ').title()}"
                    )
