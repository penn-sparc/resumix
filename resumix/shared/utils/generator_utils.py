...
class GeneratorUtils:
    ...
    def process_section(section: SectionBase) -> dict:
        ...
        try:
            ...
            if json_data:
                ...
                logger.warning(
                    "No JSON data found, falling back to raw text processing"
                )
    ...
    def convert_sections_to_generator_format(sections: Dict[str, SectionBase]) -> dict:
        ...
        for section_name, section_obj in sections.items():
            try:
                ...
                normalized_name = section_mappings.get(
                    section_name.lower(), section_name.lower()
                )

                # Process the section
                # processed_data = GeneratorUtils.process_section(section_obj)
                rewritten_text = (
                    section_obj.rewritten_text.strip()
                    if section_obj.rewritten_text
                    else ""
                )

                logger.warning(f"convert: {rewritten_text[:100]}...")
                processed_data = JsonParser.parse_string(rewritten_text)

                logger.info(processed_data)

                # Map to the appropriate resume section
                if normalized_name == "basics":
                    resume_data["basics"].update(
                        GeneratorUtils._normalize_basics(processed_data)
                    )
                    ...
                    logger.warning(
                        f"Unknown section type: {section_name}, storing as raw content"
                    )
    ...
    def _normalize_work(data: dict) -> List[dict]:
        ...
        if data.get("highlights") and isinstance(data["highlights"], list):
            ...
            summary = data.get(
                "summary", data.get("description", data.get("content", ""))
            )
