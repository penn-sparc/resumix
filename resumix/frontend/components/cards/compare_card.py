...
class CompareCard(BaseCard):
    ...
    def parse_final_answer_json(self, text: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"🔍 Debug: Parsing text: {text[:200]}... {type(text)}")

            # Method 1: Extract JSON part after "Final Answer:"
            if "Final Answer:" in text:
                json_part = text.split("Final Answer:")[-1].strip()
                logger.info(
                    f"🔍 Debug: JSON part after 'Final Answer:': {json_part[:200]}..."
                )
            ...
            # Method 2: Look for JSON block in ```json format
                ...
                if json_match:
                    ...
                    logger.info(
                        f"🔍 Debug: JSON from ```json block: {json_part[:200]}..."
                    )
                    parsed_data = json.loads(json_part)
                    logger.info(
                        f"🔍 Debug: Successfully parsed JSON from block: {parsed_data}"
                    )
                    return parsed_data
                    ...
                    for i, char in enumerate(text[start_idx:], start_idx):
                        if char == "{":
                            ...
                            if brace_count == 0:
                                ...
                                logger.info(
                                    f"🔍 Debug: JSON object found: {json_part[:200]}..."
                                )
                                parsed_data = json.loads(json_part)
                                logger.info(
                                    f"🔍 Debug: Successfully parsed JSON object: {parsed_data}"
                                )
                                return parsed_data
            ...
            # Method 4: Try to parse the entire text as JSON
                ...
                logger.info(
                    f"🔍 Debug: Successfully parsed entire text as JSON: {parsed_data}"
                )
                return parsed_data
